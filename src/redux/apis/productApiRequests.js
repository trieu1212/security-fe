...
export const getAllProduct = async(dispatch,limit,page,categoryId) =>{
    ...
    try {
        if(categoryId){
            const res = await axios.get(`http://localhost:7000/api/product/?limit=${limit}&page=${page}&categoryId=${categoryId}`);
            dispatch(getAllProductSuccess(res.data))
        }
        else{
            const res = await axios.get(`http://localhost:7000/api/product/?limit=${limit}&page=${page}`);
            dispatch(getAllProductSuccess(res.data))
        }
    } catch (error) {
